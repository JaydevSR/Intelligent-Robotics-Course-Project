...
TIME_STEP = 64
MAX_SPEED = 6.28
GOAL_POSITION = [0.2, 0.8, 0.0]
POS_EPSILON = 0.07  # distance from goal when to stop
OBS_PROX = 100.0
ANGLE_EPSILON = 0.05
...
while robot.step(TIME_STEP) != -1:
    ...
    if state == 'begin':
        ...
        aligned_to_goal = angle_of(current_position, GOAL_POSITION) > 0.98*current_angle
        aligned_to_goal = aligned_to_goal and angle_of(current_position, GOAL_POSITION) < 1.02*current_angle
        ...
        on_line_again = on_line(current_position, start_position, GOAL_POSITION,
                                tolerance=POS_EPSILON)
        not_hit_point = distance_between(current_position, hit_point) > 1.5*POS_EPSILON
        if on_line_again and not_hit_point:
